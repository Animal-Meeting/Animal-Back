...
cd /home/ubuntu/bluegreenapp

# 환경변수 DOCKER_APP_NAME을 animal-meeting으로 설정
DOCKER_APP_NAME=animal-meeting-blue-green
...
if [ -z "$EXIST_BLUE" ]; then
  ...
  if [ -z "$BLUE_HEALTH" ]; then
    # slack으로 알람을 보낼 수 있는 스크립트를 실행한다.
     echo "blue 배포 도중 실패 : $(date +%Y)-$(date +%m)-$(date +%d) $(date +%H):$(date +%M):$(date +%S)" >> /home/ubuntu/deploy.log
...
# blue가 실행중이면 green up
  ...
  if [ -z "$GREEN_HEALTH" ]; then
      ...
      echo "green 배포 도중 실패 : $(date +%Y)-$(date +%m)-$(date +%d) $(date +%H):$(date +%M):$(date +%S)" >> /home/ubuntu/deploy.log
  else
